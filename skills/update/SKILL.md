...
2. 如果 alias 不存在：
   ...
   - 写入 `~/.claude/commands/pua.md`，内容如下：
...
- **无参数** 或任意任务描述 → 加载 `pua:auto` skill
- **l1** → 加载 `pua:l1` skill
- **l2** → 加载 `pua:l2` skill
- **l3** → 加载 `pua:l3` skill
- **update** → 加载 `pua:update` skill
- **version** → 加载 `pua:version` skill

## 执行规则

1. 先识别参数属于哪个路由
2. 用 Skill tool 加载对应 skill
3. 如果有 `$ARGUMENTS` 里除了路由关键词之外的内容，作为任务描述传给 skill
```

3. 如果 alias 已存在：
   - 不要覆盖
   - 只说明已跳过 alias 写入
4. 再检查 `~/.claude/plugins/pua` 是否存在。
