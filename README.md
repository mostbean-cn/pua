...
这是一个极简版 Claude Code 插件，提供 8 个入口：

- `/pua:auto`：默认自适应强度
- `/pua:design`：设计原则提炼、界面/交互方案与多方向探索
- `/pua:l1`：最小执行纪律
- `/pua:l2`：系统排查
- `/pua:l3`：攻坚与结构化失败报告
...
### `/pua:design`
设计模式。适合界面/交互/原型、设计评审、设计原则提炼与多方案探索。强调先补齐上下文，再给最小可讨论版本，并围绕视觉、布局、流程继续迭代。
...
- `/pua:update`：源码安装会尝试 `git pull`，marketplace 安装会提示执行标准更新命令；同时会覆盖更新 `~/.claude/commands/pua.md`，让裸 `/pua` 默认等价于 `/pua:auto`。
...
执行 `/pua:update` 时，它会覆盖更新 `~/.claude/commands/pua.md`，确保裸 `/pua` 默认等价于 `/pua:auto`。
